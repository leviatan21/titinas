        ...
        <div class="mt-8">
            ...
            <article class="md:flex">
                ...
                <div class="md:w-4/5 ml-4 mt-4 d-flex flex-column">
                    ...
                    --}}
                    <ul>
                    @foreach ($step['description'] as $description)
                        <li>{!!$description!!}</li>
                    @endforeach
                    </ul>
                    <br />
                    <ul class="text-center mt-auto">
                    @foreach ($step['prices'] as $price)
                        <li class="block">{!!$price!!}</li>
                    @endforeach
                    </ul>
                </div>

                @if(!empty($step['schemamarkup']))
                @include('components.schemamarkup', ['schemamarkup' => $step['schemamarkup'] ])
                @endif
            </article>
